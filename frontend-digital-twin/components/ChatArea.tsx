      ...
      >
        {messages.length === 0 && (
          <div className="h-full flex flex-col items-center justify-center text-center max-w-md mx-auto">
             <div className="w-16 h-16 rounded-3xl bg-indigo-500/20 flex items-center justify-center mb-6 border border-indigo-500/30">
               <img src={activeTwin.avatar} className="w-12 h-12 rounded-2xl grayscale" />
             </div>
             <h2 className="text-xl font-bold text-white mb-2">Initialize {activeTwin.name}</h2>
             <p className="text-zinc-500 text-sm">
                This agent is active as a <span className="text-indigo-400 font-medium">{activeTwin.role}</span>. 
        ...
        {messages.map((msg) => (
          ...
          >
            ...
            <div className={`max-w-[85%] md:max-w-[70%] space-y-2 group relative`}>
                ...
                {msg.sender === 'assistant' && (
                  <button 
                    ...
                    className={`absolute -right-10 top-2 p-1.5 rounded-lg border transition-all opacity-0 group-hover:opacity-100 ${
                      saveStatus[msg.id] ? 'bg-emerald-500/20 border-emerald-500 text-emerald-500' : 'bg-zinc-800 border-zinc-700 text-zinc-500 hover:text-indigo-400'
                    }`}
              ...
              <div className={`text-[10px] text-zinc-600 px-1 ${msg.sender === 'user' ? 'text-right' : 'text-left'}`}>
        ...
        {activeTwin.status === TwinStatus.THINKING && (
          <div className="flex gap-4 justify-start animate-in fade-in slide-in-from-bottom-2 duration-300">
             <div className="w-8 h-8 rounded-lg overflow-hidden shrink-0 border border-zinc-800 flex items-center justify-center bg-zinc-900">
                <div className="w-4 h-4 rounded-full border-2 border-zinc-600 border-t-indigo-500 animate-spin" />
             </div>
             <div className="bg-zinc-900/50 border border-zinc-800 p-3 rounded-2xl rounded-tl-none italic text-zinc-500 text-xs">
      ...
      <div className="p-4 md:p-6 bg-gradient-to-t from-zinc-950 via-zinc-950 to-transparent">
        <div className="max-w-4xl mx-auto relative">
          <form 
            ...
            className="bg-zinc-900/80 backdrop-blur-md border border-zinc-800 rounded-2xl p-2 shadow-2xl flex items-center gap-2 group focus-within:ring-2 focus-within:ring-indigo-500/20 transition-all"
          >
            <button 
              type="button" 
              onClick={() => setIsToolMenuOpen(!isToolMenuOpen)}
              className={`p-2 rounded-xl transition-all flex items-center gap-2 ${isToolMenuOpen ? 'bg-indigo-600/20 text-indigo-400' : 'text-zinc-500 hover:text-zinc-300'}`}
            >
            ...
            <input 
              ...
              className="flex-1 bg-transparent border-none focus:ring-0 text-sm text-zinc-200 placeholder-zinc-600 py-2"
            />
            
            <button 
              type="submit"
              disabled={!input.trim() || activeTwin.status === TwinStatus.THINKING}
              className="p-2 bg-indigo-600 text-white rounded-xl hover:bg-indigo-500 disabled:opacity-50 transition-all flex items-center gap-2 pr-3"
            >
