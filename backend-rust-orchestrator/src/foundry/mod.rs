...
"#,
        ...
        let _ = service.event_bus.send(PhoenixEvent::BroadcastDiscovery {
            source: "foundry".to_string(),
            discovery_type: "agent_forged".to_string(),
            details: format!("agent:{}", manifest.name),
    ...
    async fn forge_tool(
        ...
        let _ = service.event_bus.send(PhoenixEvent::BroadcastDiscovery {
            source: "foundry".to_string(),
            discovery_type: "tool_forged".to_string(),
            details: format!("tool:{}", request.metadata.name),
            timestamp: Utc::now().to_rfc3339(),
        });
